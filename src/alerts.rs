    ...
    let groups = METRICS
        ...
        .flat_map(|alert| {
            alert
                .success_rate
                .iter()
                .map(|success_rate| SuccessRateObjective {
...
trait Objective {
    ...
    fn success_query(&self, window: &str) -> String {
        let error_rate = self.error_query(window);
        let request_rate = self.total_query(window);
    ...
    fn filter_labels(&self) -> String {
        ...
        format!("{{function=\"{function}\", module=\"{module}\"}}")
    }
  ...
  rules:\n"
        ...
        for window in WINDOWS {
            let query = self.success_query(window);
            rules.push_str(&format!(
                "  - record: slo:sli_success:ratio_rate{window}
    expr: {query}
    {labels}
      window: {window}\n"
            ));
  ...
  - record: slo:current_burn_rate:ratio
    expr: slo:sli_error:ratio_rate5m{filter_labels} / on(function, module) group_left slo:error_budget:ratio{filter_labels}
    {recording_labels}
  - record: slo:period_burn_rate:ratio
    expr: slo:sli_error:ratio_rate30d{filter_labels} / on(function, module) group_left slo:error_budget:ratio{filter_labels}
    {recording_labels}
