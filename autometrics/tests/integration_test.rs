...
fn single_function() {
    ...
    let call_count_metric: Regex = Regex::new(
        r#"function_calls_count\{\S*function="test_fn"\S*module="integration_test"\S*\} 2"#,
    )
    .unwrap();
    let duration_metric: Regex = Regex::new(
        r#"function_calls_duration_bucket\{\S*function="test_fn"\S*module="integration_test"\S*\}"#,
    )
    .unwrap();
...
fn result() {
    ...
    #[autometrics]
    fn test_fn(should_error: bool) -> Result<(), ()> {
        if should_error {
            Err(())
        } else {
            Ok(())
    ...
    let error_count: Regex =
        Regex::new(r#"function_calls_count\{\S*result="error"\S*\} 2"#).unwrap();
    assert!(error_count.is_match(&metrics));
    let ok_count: Regex = Regex::new(r#"function_calls_count\{\S*result="ok"\S*\} 1"#).unwrap();
...
fn ok_if() {
    ...
    let metrics = autometrics::encode_global_metrics().unwrap();
    let error_count: Regex =
        Regex::new(r#"function_calls_count\{\S*result="error"\S*\} 1"#).unwrap();
    assert!(error_count.is_match(&metrics));
}
